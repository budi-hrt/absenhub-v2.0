...
<body>
    ...
    <div class="sub">Periode: {{ $formatTanggalAwal }} s.d {{ $formatTanggalAkhir }}</div>

    <div class="filter-info">
        <table>
            <tr>
                <td style="width: 50%">
                    <span>Status Absen:</span> {{ $keterangan ?: 'Semua Status' }}
                </td>
                <td style="width: 50%">
            ...
            <tr>
                <td>
                    <span>Total Baris Data:</span> {{ $absens->count() }}
                </td>
                <td>
        ...
        <table class="rekap-grid">
            <tr>
                <td style="background-color: #fee2e2; color: #991b1b; width: 25%">
                    Alpa: <span class="rekap-value">{{ $rekap['alpa'] }}</span>
                </td>
                <td style="background-color: #fef3c7; color: #92400e; width: 25%">
                    Sakit: <span class="rekap-value">{{ $rekap['sakit'] }}</span>
                </td>
                <td style="background-color: #ede9fe; color: #5b21b6; width: 25%">
                    Cuti: <span class="rekap-value">{{ $rekap['cuti'] }}</span>
                </td>
                <td style="background-color: #f1f5f9; color: #475569; width: 25%">
                    Izin: <span class="rekap-value">{{ $rekap['izin'] }}</span>
                </td>
            </tr>
        </table>
    </div>
        ...
        <tbody>
            @forelse ($absens as $i => $row)
                ...
                <tr>
                    <td>{{ $i + 1 }}</td>
                    <td class="text-left">
                        <strong>{{ $row->karyawan->nama_karyawan }}</strong>
                    </td>
                    <td class="text-left">
                        {{ $row->karyawan->jabatan?->nama_jabatan ?? '-' }}
                    </td>
                    <td>{{ $row->tanggal_absen->format('d/m/Y') }}</td>
                    <td class="{{ $bgColor }}">
                        {{ $row->keterangan }}
                    </td>
                    <td style="font-family: monospace;">{{ $row->scan_in ?? '-' }}</td>
                    <td style="font-family: monospace;">{{ $row->scan_out ?? '-' }}</td>
